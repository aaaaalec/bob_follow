...
from omni.isaac.bob.controllers.rmpflow_controller import RMPFlowController
from omni.isaac.bob.tasks import FollowTarget as FollowTargetTask
import numpy as np
...
class FollowTarget(BaseSample):
    ...
    async def setup_post_load(self):
        self._bob_task = list(self._world.get_current_tasks().values())[0]
        self._task_params = self._bob_task.get_params()
        my_bob = self._world.scene.get_object(self._task_params["robot_name"]["value"])
        self._controller = RMPFlowController(name="target_follower_controller", robot_articulation=my_bob)
        self._articulation_controller = my_bob.get_articulation_controller()
        self._fixed_target_position = np.array([0.1, 0.0, 0.1])
        self._fixed_target_orientation = np.array([1.0, 0.0, 0.0])
        return
    ...
    def _on_follow_target_simulation_step(self, step_size):
        ...
        #actions = self._controller.forward(
        #    target_end_effector_position=self._fixed_target_position,
        #)
